...
def parse_message(message):
    """
    Parses a structured message into its components.
    Assumes the message is formatted as 'command length [content]'.

    :param message: The entire message string to be parsed.
    :return: A tuple containing command, length as integer, and content as string.
    """
    parts = message.split(' ', 2)  # Split into at most three parts
    command = parts[0]
    length = int(parts[1]) if len(parts) > 1 else 0
    content = parts[2] if len(parts) > 2 else ""
    return command, length, content